...
const Progress = () => {
  ...
  const handleCheckboxChange = (isChecked: boolean, label: string) => {
    // Set the global and async progress status
    if (dayStatus) {
      const newCheckBoxes = dayStatus.progress.checkboxes.map((item) =>
  ...
  const resetProgressBoxes = () => {
    if (dayStatus) {
      const newDayStatus = {
        ...dayStatus,
        progress: {
          ...dayStatus.progress,
          checkboxes: dayStatus.progress.checkboxes.map((item) => ({
            ...item,
            isChecked: false,
          })),
          dayStatus: 0,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        <View style={{ paddingBottom: 50 }}>
          ...
          {dayStatus.progress.checkboxes.map((item, index) => (
            <CheckboxWithLabel
              key={index}
              label={item.label}
